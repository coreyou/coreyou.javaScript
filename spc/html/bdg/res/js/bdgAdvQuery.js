/**
 * Created by I27160 on 2014/7/21.
 */
  ...
  $.extend(_bd, {
    aq: {
      getRSInfo: function (type) {
        var info = {
          type: type,
          mustWhere: _bd.aq.getAQCCondition()
        };
        if (_bd.designer) {
          info.tableId = _bd.reface.getTables();
          info.relation = _bd.reface.getRel();
          info.limitCol = _bd.reface.getLimitCol('obj');
        } else if (_bd.spc) {
          var infoId = _bd.spc.getActiveInfoId(),
              infoStore = $('#bas-store').data('info') || {},
              isi = infoStore[infoId];
          if (isi) {
            var uv = $('#' + _bd.spc.getActiveChart()).data('uv');
            info.canvasId = isi.INFOID;
            info.tableId = $.parseJSON(isi.TABLEID);
            if (isi.RELATION) info.relation = $.parseJSON(isi.RELATION);
            if (isi.LIMITCOL) {
              info.limitCol = _bd.spc.getLimitColInfo($.parseJSON(isi.LIMITCOL));
      ...
      getResultSet: function (type) {
        _bd.util.getJSON('bdgcLogic.getLCUCResult', _bd.aq.getRSInfo(type), function (data) {
          _bd.aq.showUVTable(type, data);
          if (_bd.spc && data.result) {
...
//        $('#bdg-adv-query-rs').children().remove();
        if (rs) {
          if (_bd.designer) lc = _bd.reface.getLimitCol('obj');
          else if (_bd.spc) lc = _bd.spc.getLimitColInfo();

          var g = '<div style="height:4px;"></div><div is="jqgrid" id="' + boxId + '-grid" autofit="true" headervisible="false" width="800" height="480" ' +
                  'onrow="_bd.aq.chartPrevByUV(' + isUC + ')" filtertoolbar="true" checkbox="false" page="false" stripe="false">';
...
//            if (isUC) {
//              if (isUK) {
//                tbDom += "<th class='text-center'>" + v.colId + "</th>";
//                uc += v.colId;
//              }
//            } else {
//              tbDom += "<th class='text-center'>" + v.colId + "</th>";
...
//            }
//          });
//          tbDom += "</tr></thead><tbody>";
//          $.each(rs, function (tri, trv) {
//            var uvPara = '';
//            $.each(trv, function (tdi, tdv) {
//              if (uc.toUpperCase().indexOf(tdi.toUpperCase()) !== -1) {
//                uvPara += tdv + ';';
//              }
//            });
//            tbDom += "<tr style='cursor:pointer;' onclick='_bd.aq.chartPrevByUV(\"" + uvPara.slice(0, -1) + "\")'>";
//            $.each(trv, function (tdi2, tdv2) {
//              tbDom += "<td>" + tdv2 + "</td>";
//            });
//            tbDom += "</tr>";
//          });
...
//          $table.append("<th class='text-center'>查無資料！</th></tr></thead>");
      ...
      aqResultSet: function (type) {
        _bd.aq.closeCommonData();
        _bd.aq.storeAQC();
        _bd.aq.getResultSet(type);
      },
      ...
      openAI: function (trigger) {
        ...
        _bd.util.getJSON('bdgcDMA.getAllQuerySet', {}, function (data) {
          _bd.aq.showAITable(data);
        });
      },
      ...
      showAITable: function (data) {
        ...
        if (data) {
          ...
          $.each(data, function (i, v) {
            tbDom += "<tr style='cursor:pointer;' onclick='_bd.aq.setAssist(\"" + triggerId + "\", \"" + v.assistId + "\")'>";
      ...
      setAdvQuery: function (data) {
        ...
        if (gData && gData.length > 0) {
          $form.append(_bd.aq.genAdvQuery(gData));
          _bd.aq.setupDatePicker();
          _bd.aq.setAQC();
        }
      },
      ...
      setAQC: function () {
        ...
        if (_bd.spc) {
          data = $('#' + _bd.spc.getActiveChart()).data('aqc');
        } else if (_bd.designer) {
      ...
      genAdvQuery: function (data) {
        ...
        $.each(data, function (i, v) {
          ...
          if (!isShow) {
            if (!_bd.aq.isDateType(type)) {
              if (type === 'INPUT') {
                input = '<input type="text" class="form-control" placeholder="請輸入資料值" data-aqc="' + col + '">';
              } else {
                input = '<span class="input-group-addon" onclick="_bd.aq.showCommonData(this, \'_bd.aq.setAdvQueryCol\')"><span title="輔助搜尋" class="glyphicon glyphicon-filter"></span></span>' +
                        '<input type="text" class="form-control" placeholder="點擊前端按鈕，查詢輔助輸入資料清單" data-key="' +
                        type + '" data-aqc="' + col + '">';
              }
              input += '<span class="input-group-addon" onclick="_bd.aq.removeAQC(this)"><span title="清空輸入值" class="glyphicon glyphicon-remove"></span></span>';
            } else {
              input = _bd.aq.getBootIcon(v.inputType) + '<input type="text" class="form-control" placeholder="請點擊操作日期、時間挑選器" data-aqc="' + col + '">' +
            ...
            row =
              '<div class="form-group col-lg-12">' +
                ...
                _bd.aq.getBootDateTimePicker(v.inputType) + input +
                '</div>' +
              '</div>';
            dom += row;
          }
      ...
      getCommonData: function (onRowCallback, dataKey, noNav) {
        _bd.util.getJSON('bdgcDMA.getQuerySet', {assistId: dataKey, filterWay: 'FilterList'}, function (data) {
          _bd.aq.genGrid(data, onRowCallback, noNav);
        });
      },
                    ...
                    '<nav class="navbar navbar-toolbar" role="navigation">' +
                      '<div class="container-fluid">' +
                        '<div class="btn-toolbar" role="toolbar">' +
                          '<div class="btn-group">' +
                            '<button id="bdg-common-data-range-symbol" class="btn btn-default" onclick="_bd.aq.setAQCRangeSymbol()">區間符號</button>' +
                    '</div></div></div></nav>',
      ...
      showCommonData: function (trigger, onRowCallback, assistId, noNav) {
        ...
        _bd.aq.getCommonData(onRowCallback, key, noNav);
      },
      ...
      aqDataOrder: function () {
        var $colGrid = $('#bdg-aq-data-order-grid'),
            ...
            data = [];
        if (_bd.designer) data = $('#bdg-lc-tar-grid').gk('list');
        else if (_bd.spc) data = _bd.spc.getLimitColInfo();
