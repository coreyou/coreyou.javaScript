angular.module('bdgApp').controller('bdgDesignerCtrl', bdgDesignerCtrlFunc);
function bdgDesignerCtrlFunc($scope, $http, $icscRest, $modal, uiGridConstants) {
  "use strict";
  $scope.designer = {
    dataPrev: function () {
      $http.post('bdgcLogic.getPreviewData', params).success(function (data) {
        //_bd.designer.dataView(data);
        $scope.statProcess.showStatTable = false;
        $scope.statProcess.dataBtnLabel = '分佈圖型';
      });
    },
    advChartPrev: function (suv) {
      ...
      $icscRest.post('bdg/logic/getChartData', info).success(function (data, status, headers) {
        ...
        if (result === "Success") {
          ...
          if (uv && uv.length > 0) {
            _bdg.reface.storeUV(uv);
            _bdg.reface.storeUniqueCol('val', uv);
          } else {
            data.defaultUK = $('#bdg-store').data('uv');
          }
          _bdg.designer.advChartView(data);
          //$chart.show();
        } else {
  ...
  $.extend(_bdg, {
    designer: {
      removeInfo: function (trigger) {
        ...
        if (tarData) {
          $grid.gk('del', tarId);
          _bdg.util.getJSON('bdgcLogic.removeInfo', {infoId: tarData.infoid}, function (data) {
            console.log(data);
          });
        }
      },
      ...
      registerEvents: function () {
        _bdg.designer.checkStep1();
      },

      getMainTable: function () {
        var table = _bdg.reface.getTables();
        if (table) return table[0];
        else return false;
      },

      step2AdjustGrid: function () {
        var height = _bdg.util.detectHeight('window', 118),
            width = _bdg.util.detectWidth('.bdg-content', 0);
        _bdg.util.adjustSize('#bdg-axes-setup-src', height * 0.7, width);
        _bdg.util.adjustSize('#bdg-axes-setup-tar', height * 0.3, width);
        $(window).on('resize', function () {
          var winHeight = _bdg.util.detectHeight('window', 118);
          _bdg.util.adjustSize('#bdg-axes-setup-src', winHeight * 0.7, _bdg.util.detectWidth('.bdg-content', 0));
          _bdg.util.adjustSize('#bdg-axes-setup-tar', winHeight * 0.3, _bdg.util.detectWidth('.bdg-content', 0));
        });
      },

      step2: function () {
        _bdg.designer.step2AdjustGrid();
      },
      ...
      inAxes: function () {
        _bdg.designer.step2GridTuning();
        _bdg.designer.step2GridTuning();
      },

      // Common Data Usage
      dataSrc: function (target) {
        _bdg.reface.decideMainTable(target);
      ...
      queryTables: function () {
        ...
        $grid.gk('clear');
        _bdg.util.getJSON('bdgcDMA.getTable', {domain: domain, keyword: ''}, function (data) {
          $grid.gk('add', data);
        });
      },

      pickRel: function (trigger) {
        var tbName = _bdg.designer.getMainTable(),
        ...
        if (tbName) {
          ...
          $grid.gk('clear');
          _bdg.util.getJSON('bdgcDMA.getRelation', {srcTable: tbName}, function (data) {
            var rel = [];
      ...
      paintSubTable: function (target) {
        var mt = _bdg.designer.getMainTable(),
            $grid = $(target),
      ...
      getColForMake: function (type, data) {
        ...
        $.each(data, function (idx, val) {
          if (val.table && val.dataType) {
            if (_bdg.util.isStrDataType(val.dataType.toLowerCase())) {
              strList.push(val);
            } else {
              numList.push(val);
            }
      ...
      makeNumCol: function () {
        ...
        $colGrid.gk('clear');
        $colGrid.gk('add', _bdg.designer.getColForMake('num', $srcGrid.gk('list')));
      ...
      makeStrCol: function () {
        ...
        $colGrid.gk('height', 200);
        $colGrid.gk('clear');
        $colGrid.gk('add', _bdg.designer.getColForMake('str', $srcGrid.gk('list')));
      ...
      makeNumColAct: function (actType, val) {
        ...
        switch (actType) {
          ...
          case "resetSub":
            ...
            break;
          case "addSub":
            if (subVal !== '' && !_bdg.util.isOp(subVal)) {
        ...
        switch (act) {
          ...
          case "input":
            var $input = $('#bdg-essential-col-input'),
                inputVal = $input.val();
            if (_bdg.util.isRuleOp(ruleVal)) {
              if (_bdg.util.isStrDataType($colGrid.data('colDataType'))) {
...
//                $msg.text('請由輸入器輸入右運算元！');
          ...
          default:
            var tarId = $(act).closest('tr').attr('id');
            $ruleGrid.gk('del', tarId);
            _bdg.designer.storeDataExtract();
            $msg.text('');
            break;
        }
      },
      ...
      storeAxis: function (type, data) {
        ...
        switch (type) {
          ...
          case "YL":
            ...
            if (!listYL) listYL = [];
            if ($scope.lineProcess.active && !data.formula) data.func = "SUM";
            listYL.push(data);
      ...
      addAxis: function (axis) {
        ...
        srcData.axis = _bdg.util.getAxisName(axis);
        if ($scope.lineProcess.active && axis.slice(0, 1) === 'Y' && !srcData.formula) srcData.func = "SUM";
...
//          axis: '請選擇'
//        });
        _bdg.designer.removeAxis(_bdg.util.getAxisKey(tarData.axis), tarData.table, tarData.colId);
        _bdg.designer.paintSubTable('#bdg-axes-setup-src-grid');
      ...
      setAxisInfo: function () {
        ...
        switch (axis) {
          case 'X':
            _bdg.designer.storeAxis(axis, colRow);
            _bdg.designer.addAxis(axis);
            break;
          case 'YL':
            _bdg.designer.storeAxis(axis, colRow);
            _bdg.designer.addAxis(axis);
      ...
      setEssential: function () {
        ...
        $ruleGrid.gk('clear');
        $ruleGrid.gk('add', _bdg.reface.getEssential('arr'));
      },

      dataPrev: function () {
        var params;
        _bdg.designer.storeDataExtract();
        params = _bdg.reface.getDataPrevInfo();
        // info verify -- check the parameters
        var verify = _bdg.designer.infoVerify(params);
        if (!verify.approved) {
          bootbox.alert(verify.warning);
          return;
        }
        $http.post('bdgcLogic.getPreviewData', params).success(function (data) {
          _bdg.designer.dataView(data);
          $scope.statProcess.showStatTable = false;
          $scope.statProcess.dataBtnLabel = '分佈圖型';
        });
      },
      ...
      dataView: function (data) {
        ...
        if (exeResult === 'Success') {
          ...
          $('#bdg-data-view').show();
          $('#bdg-chart-view').hide();
          _bdg.util.showScroll('#bdg-data-view-dialog');
        } else {
          $dialog.dialog({
            width: 950,
            height: 650,
          ...
          _bdg.designer.turnOnDataView(false);
        }
      },
      ...
      registerChartResize: function (target) {
        $(target).closest('.ui-dialog').on('resize', function (evt) {
          ...
          _bdg.drawer.redraw('#bdg-chart-view-canvas', data);
        });
      },
      ...
      turnOnDataView: function (doPrev) {
        ...
        $('#bdg-chart-view').hide();
        _bdg.util.showScroll('#bdg-data-view-dialog');
        if (doPrev) _bdg.designer.dataPrev();
      },
      ...
      turnChartDataView: function () {
        ...
        if (viewText === '圖型預覽') {
          ...
          $('#bdg-data-view').hide();
          $('#bdg-chart-view').show();
          _bdg.designer.chartPrev();
          _bdg.designer.registerChartResize('#bdg-data-view-dialog');
          _bdg.util.hideScroll('#bdg-data-view-dialog');
        } else {
          _bdg.designer.turnOnDataView(true);
        }
      },

      chartPrev: function () {
        var info = _bdg.reface.getDataPrevInfo();
        _bdg.util.getJSON('bdgcLogic.getChartData', info, function (data) {
          var result = data.exeResult,
              $dialog = $('#bdg-data-view-dialog');
          if (result === "Success") {
            _bdg.designer.chartView(data);
      ...
      infoVerify: function (info) {
        ...
        if (info.chartType === 'Line') {
          if (!x || x.length < 1) {
            ...
            $.each(y, function (i, v) {
              if (v.dataType && _bdg.util.isStrDataType(v.dataType)) {
                approved = false;
                waring = 'Y軸必須指定數值型欄位！';
              }
            });
      ...
      advChartPrev: function (suv) {
        var info = _bdg.reface.getDataPrevInfo(),
            chartTitle = _bdg.reface.chartTitle(),
        ...
        if (suv) {
          ...
          _bdg.reface.saveUC();
          $.extend(true, info, _bdg.reface.getUniqueCol());
        }
        var verify = _bdg.designer.infoVerify(info);
        if (!verify.approved) {
          bootbox.alert(verify.warning);
          return;
        }
        ...
        $('#bdg-adv-chart-prev-dialog').dialog({
          ...
          width: chartWidth,
          height: chartHeight,
          title: chartTitle + "(" + _bdg.reface.getInfoId(false) + ")"
        });
        _bdg.util.getJSON('bdgcLogic.getChartData', info, function (data) {
          var result = data.exeResult,
      ...
      advChartView: function (data) {
        ...
        $.extend(true, cd, {
          ...
          chartType: data.chartType,
          chartTitle: _bdg.drawer.genChartTitle(data),
        ...
        switch (act) {
          case "col":
            var colVal = colRow.table + '.' + colRow.colId + ' ASC',
                realObj = {axis: _bdg.reface.getPrevDataPrefix(colRow.axis), rule: colVal};
            $rule.val(colVal);
...
//        $ruleGrid.gk('clear');
      ...
      step3AdjustGrid: function () {
        var height = _bdg.util.detectHeight('window', 118),
            width = _bdg.util.detectWidth('.bdg-content', 0);
        _bdg.util.adjustSize('#bdg-lc-src', height * 0.6, width);
        _bdg.util.adjustSize('#bdg-lc-tar', height * 0.4, width);
        $(window).on('resize', function () {
          var winHeight = _bdg.util.detectHeight('window', 118);
          _bdg.util.adjustSize('#bdg-lc-src', winHeight * 0.6, _bdg.util.detectWidth('.bdg-content', 0));
          _bdg.util.adjustSize('#bdg-lc-tar', winHeight * 0.4, _bdg.util.detectWidth('.bdg-content', 0));
        });
      },

      step3: function () {
        _bdg.designer.step3AdjustGrid();
      },
      ...
      inLc: function () {
        ...
        _bdg.designer.step3GridTuning();
        _bdg.designer.step3GridTuning();
        if (lcInit) {
          _bdg.reface.limitColReload();
        } else {
          _bdg.reface.limitColInit();
        }
      },

      sqlPrev: function () {
        _bdg.util.getJSON('bdgcLogic.getFinalSql', _bdg.reface.getDataPrevInfo(), function (data) {
      ...
      step4AdjustGrid: function () {
        var height = _bdg.util.detectHeight('window', 108),
            width = _bdg.util.detectWidth('.bdg-content', 0);
        _bdg.util.adjustSize('#bdg-step4-content', height, width);
        $(window).on('resize', function () {
          _bdg.util.adjustSize('#bdg-step4-content', _bdg.util.detectHeight('window', 82), _bdg.util.detectWidth('.bdg-content', 0));
        });
      },

      step4: function () {
        _bdg.designer.step4AdjustGrid();
      }
  ...
  $scope.onResize = function (event) {
    ...
    $scope.c3Chart1.chart.resize($scope.win1.size());
  };
  $scope.$on('c3Draw', function (event, data) {
  });
}
